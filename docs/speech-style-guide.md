...
3. 如果样例变多，再用 Qdrant 做本地检索：按任务类型取 2 到 3 个最相近样例，拼进 Skill 或 MCP 准备上下文。

本地 RAG 只用于风格参考，不用于替代 Codex 对当前任务的理解。当前任务的理解仍然必须由 Codex 在调用 `codex_speak_prepare` 前完成。

语料来源、许可、清洗和验证规则见 [儿童表达语料方案](speech-corpus-plan.md)。所有外部数据必须先进入来源登记表，再经过许可门禁和人工抽样验证；产品样例优先使用原创改写，不直接复制外部长文本。
