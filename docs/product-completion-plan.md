...
- `prepare-qa-handoff.mjs` 已补齐为外部 QA 交付说明生成器，会把 release 包 sha256、安装命令、验收命令和 QA 回传要求写入 `dist/qa-handoff`；传入 `--include-ci-artifacts` 时还会附上当前成功 main CI 的 macOS/Windows smoke artifact 名称、大小、run 链接和下载 API 地址，传入 `--ci-artifacts-json` 时可离线复现 artifact handoff 渲染，降低外部测试漏步骤或找不到 Windows 包的概率。
- main CI 的 release package smoke 会上传 macOS/Windows 临时 QA artifacts，方便不打 tag 也能从最新成功 CI 下载 Windows 包给外部测试者；release readiness 会在线确认 `codex-speak-macos-ci` 和 `codex-speak-windows-ci` 存在、未过期且大小大于零；正式公开分发仍走 release workflow。
