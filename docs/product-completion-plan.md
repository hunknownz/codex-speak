...
- `prepare-qa-handoff.mjs` 已补齐为外部 QA 交付说明生成器，会把 release 包 sha256、安装命令、验收命令和 QA 回传要求写入 `dist/qa-handoff`；传入 `--include-ci-artifacts` 时还会附上当前成功 main CI 的 macOS/Windows smoke artifact 名称、大小、run 链接和下载 API 地址，降低外部测试漏步骤或找不到 Windows 包的概率。
