...
如果 MCP side-channel 不可用，Hook 不再压缩最终可见回复，也不读取用户输入或普通 final answer。它只播放一个明确提示：这次没有收到插件准备好的朗读导览。

历史 HTML/Markdown 协议只用于旧会话、`extract` 命令和排障兼容，不作为新回答和 Hook 的默认输出方式。

### 4. 播放队列

最终导览和过程提示都可能启动本地 TTS。为了避免“读到一半被下一句切断”，本地播放采用队列策略：

- 最终导览排队播放，尽量完整读完。
- 过程提示是短句，队列忙时直接跳过。
- 手动试听可以打断，因为这是用户主动操作。
- 停止按钮会中断当前朗读，并让排队等待的旧任务退出。
...
| MCP 不可用 | Hook 明示没有收到插件导览，不猜最终回复 |
