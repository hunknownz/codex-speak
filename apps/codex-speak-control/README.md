...
The default asset is an original Codex Speak tech companion generated by `scripts/generate-pet-assets.swift`. To redesign the pet, regenerate or replace `codex-agent.mov` and `codex-agent-hit.png` while keeping the same lil-style helper code path.
